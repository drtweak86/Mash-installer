    ...
    fn configure_local_mirror(&self, ctx: &mut installer_core::PhaseContext) -> anyhow::Result<()> {
        ...
        if let Ok(output) = ctx.platform.system.command_output(&mut cmd) {
            if output.status.success() {
                ctx.record_action("Mirror Heuristics: Local dnf-compatible proxy detected at localhost:3142");
                ...
                if ctx.options.dry_run {
                    ctx.record_dry_run("mirror_heuristics", "Would configure dnf proxy", Some(conf_path.to_string()));
                } else {
                    let mut append_cmd = std::process::Command::new("sh");
                    append_cmd.arg("-c").arg(format!("echo '{}' | sudo tee -a {}", proxy_line, conf_path));
