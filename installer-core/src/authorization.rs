...
use crate::{cmd, phase_runner::PhaseObserver, context::UserOptionsContext};
...
impl<'a> AuthorizationService<'a> {
    ...
    pub fn authorize(&mut self, auth_type: AuthType) -> Result<()> {
        if self.options.dry_run {
            info!("Dry run: Skipping interactive authorization for {:?}", auth_type);
            return Ok(());
        }

        if !self.options.interactive {
            warn!("Non-interactive mode: Cannot perform interactive authorization for {:?}", auth_type);
            return Ok(());
        }
    ...
    fn authorize_gh(&mut self) -> Result<()> {
        ...
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch gh auth login: {}", e))?;
    ...
    fn authorize_ssh(&mut self) -> Result<()> {
        ...
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch ssh-keygen: {}", e))?;
        
        if status.success() {
            info!("SSH key generated successfully.");
            
            if self.check_gh_auth() {
                let add_to_gh = self.observer.confirm(
                    "Would you like to add this new SSH key to your GitHub account?"
                );

                if add_to_gh {
                    let mut gh_cmd = Command::new("gh");
                    gh_cmd.args(["ssh-key", "add", "--title", "MASH Forge Key"]);
                    let gh_status = gh_cmd.status().map_err(|e| anyhow!("Failed to add SSH key to GitHub: {}", e))?;
    ...
    fn authorize_rclone(&mut self) -> Result<()> {
        ...
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch rclone config: {}", e))?;
    ...
    fn authorize_borg(&mut self) -> Result<()> {
        ...
        let should_init = self.observer.confirm(
            &format!("Would you like to initialize a new Borg repository at {}?", default_repo.display())
        );
        ...
        cmd.args(["init", "--encryption", "repokey", &default_repo.display().to_string()]);
        
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch borg init: {}", e))?;
        
        if status.success() {
            info!("Borg repository initialized successfully at {}.", default_repo.display());
            Ok(())
        } else {
    ...
    fn authorize_tailscale(&mut self) -> Result<()> {
        ...
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch tailscale up: {}", e))?;
    ...
    fn authorize_ngrok(&mut self) -> Result<()> {
        ...
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch ngrok config: {}", e))?;
    ...
    fn authorize_cloudflared(&mut self) -> Result<()> {
        ...
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch cloudflared login: {}", e))?;
        
        if status.success() {
            info!("Cloudflared authorized successfully.");
            Ok(())
        } else {
            Err(anyhow!("Cloudflared authorization failed or was cancelled."))
        }
    }
    ...
    fn authorize_docker(&mut self) -> Result<()> {
        ...
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch docker login: {}", e))?;
    ...
    fn authorize_argon(&mut self) -> Result<()> {
        ...
        let status = cmd.status().map_err(|e| anyhow!("Failed to launch argonone-config: {}", e))?;
