...
use anyhow::Result;
use installer_core::pi_overlord::{PackageCategory, PackageMapping, PiOverlord, PiOverlordPackages};
...
impl installer_core::DistroDriver for MockFedoraDriver {
    ...
    fn matches(&self, _info: &installer_core::PlatformInfo) -> bool { false }
    fn pkg_backend(&self) -> PkgBackend { PkgBackend::Dnf }
}
...
impl installer_core::DistroDriver for MockArchDriver {
    ...
    fn matches(&self, _info: &installer_core::PlatformInfo) -> bool { false }
    fn pkg_backend(&self) -> PkgBackend { PkgBackend::Pacman }
}
...
fn test_pi_overlord_packages_initialization() -> Result<()> {
    ...
    for category in expected_categories {
        assert!(packages.get_mapping(category).is_some(), 
               "Missing package mapping for {:?}", category);
    }
...
fn test_rust_toolchain_package_mapping() -> Result<()> {
    let packages = PiOverlordPackages::new();
    let rust_mapping = packages.get_mapping(PackageCategory::RustToolchain).unwrap();
...
fn test_all_categories_have_packages() -> Result<()> {
    ...
    for category in [
        ...
        if let Some(mapping) = packages.get_mapping(category) {
            ...
            assert!(!fedora_pkgs.is_empty() || !debian_pkgs.is_empty() || !arch_pkgs.is_empty(),
                   "Category {:?} has no packages for any distro", category);
        }
    }
    ...
    if let Some(cargo_mapping) = packages.get_mapping(PackageCategory::CargoTools) {
        ...
        assert!(fedora_pkgs.is_empty(), "CargoTools should have no Fedora packages");
        assert!(debian_pkgs.is_empty(), "CargoTools should have no Debian packages");
        assert!(arch_pkgs.is_empty(), "CargoTools should have no Arch packages");
    }
